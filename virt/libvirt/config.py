...
class LibvirtConfigGuestSpiceChannel(LibvirtConfigGuestDevice):

    def __init__(self, **kwargs):
        super(LibvirtConfigGuestSpiceChannel, self).__init__(
                                                      root_name="channel",
                                                      **kwargs)
        self.type = "spicevmc"
        self.target_type = "virtio"
        self.target_name = "com.redhat.spice.0"
        self.alias_name = "virserial-channel1"

    def format_dom(self):
        dev = super(LibvirtConfigGuestSpiceChannel, self).format_dom()
        dev.set("type", self.type)
        drv_target = etree.Element("target")
        drv_target.set("type", self.target_type)
        drv_target.set("name", self.target_name)
        dev.append(drv_target)
        drv_alias = etree.Element("alias")
        drv_alias.set("name", self.alias_name)
        dev.append(drv_alias)
        return dev
...
class LibvirtConfigGuestVideo(LibvirtConfigGuestDevice):
    ...
    def format_dom(self):
        ...
        drv_model = etree.Element("model")
        drv_model.set("type", self.type)
        drv_model.set("vram", self.vram)
        drv_model.set("heads", self.heads)
        dev.append(drv_model)
        return dev
...
class LibvirtConfigGuestGraphics(LibvirtConfigGuestDevice):
    ...
    def format_dom(self):
        ...
        drv_streaming = etree.Element("streaming")
        drv_streaming.set("mode", self.streaming_mode)
        dev.append(drv_streaming)
